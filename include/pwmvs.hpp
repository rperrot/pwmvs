...
private:
  ...
  {
    DirectionTB( int rows, int cols )
        : Direction( rows, cols )
    {
    }

    virtual int dim1() const override
    {
      return cols;
    }
    virtual int dim2() const override
    {
      return rows;
    }
...
protected:
  ...
  int bestHypothesis( const Hypothesis ( &hypotheses )[ N ], const std::vector<FloatT>& probabilities, const Vector2i& x, _UniformRandomNumberGenerator& urng, Image<FloatT>& hypothesis_photoconsistency_cache )
  {
    ...
    std::fill( evaluations, evaluations + N, 0 );

    Sampler sampler( probabilities );

    PhotometricConistencyCache cache;
    photometric_consistency_calculator.computeCache( x, cache );
    ...
    for ( int i = 0; i < options.monte_carlo_samples; i++ )
    {
      ...
      for ( int h = 1; h < N; ++h )
      {
        ...
        {
          photo_consistency                              = photometric_consistency_calculator( *src, x, hypotheses[ h ].normal, hypotheses[ h ].depth , cache );
...
private:
  ...
  std::vector<Image<FloatT>, Eigen::aligned_allocator<Image<FloatT>>> color_similarities; // NCC

  std::vector<Image<FloatT>, Eigen::aligned_allocator<Image<FloatT>>> backward_messages;

  std::vector<Image<FloatT>, Eigen::aligned_allocator<Image<FloatT>>> selection_probabilities;
  std::vector<Image<FloatT>, Eigen::aligned_allocator<Image<FloatT>>> prev_selection_probabilities;
