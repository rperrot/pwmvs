...
using FloatT   = float;
using Normal   = openMVG::Vec3f; // Eigen::Matrix<FloatT, 3, 1>;
...
//EIGEN_DEFINE_STL_VECTOR_SPECIALIZATION( Normal );
...
{
  View( int w, int h )
      : height( h ), width( w ), K( Matrix3::Identity() ), K_inv( Matrix3::Identity() )
  {
  ...
  {
    return x.x() >= 0 && x.x() < width && x.y() >= 0 && x.y() < height;
  }
...
struct SrcView : public View
{
  ...
  Vector3 unproject( const Vector2& x, const FloatT depth ) const override
  {
    return P_inv * ( depth * x.homogeneous() ).homogeneous();
  }

  Normal projectNormal( const Normal& n ) const override
  {
    return ( R * n ).normalized();
  }

  using View::unprojectNormal;
  Normal unprojectNormal( const Normal& n ) const override
  {
    return ( R.transpose() * n ).normalized();
  }

  FloatT distance( const Vector3& X ) const override
  {
